...
# =============================================================================
# Builds host-microbe association networks from the intermediates produced
# by 1_load_all_categories_additional.R. Two layouts are emitted:
#   - comprehensive: full significant set
#   - compact: top K taxa per effect direction (by |estimate|)
#
...
# (igraph, ggraph, tidygraph, dplyr, etc.).
...
tables_dir <- file.path(output_path, "supplementary_tables")
...
cat("  Compact top-K per direction:", COMPACT_TOP_K_PER_DIRECTION, "\n")
cat("  PDF font family:", pdf_font_family, "\n")
cat("  Data (intermediate):", data_path, "\n")
cat("  Output root:", output_path, "\n\n")
...
if (!dir.exists(data_path)) {
  stop("Intermediate data not found. Run scripts/8_network_analyses/1_load_all_categories_additional.R first.\nMissing: ", data_path)
}
...
run_network_dataset <- function(ds_name, sig_res_table, ps, trans,
                                network_mode = "comprehensive",
                                k_per_dir = COMPACT_TOP_K_PER_DIRECTION) {

  message("== Network analysis: ", nice(ds_name), " (", trans, ", ", network_mode, ")")

  keep <- keep_taxa(ps, min_prevalence, trans)
  if (!length(keep)) {
    message("   no taxa left")
    return(list(plot_list = list(), compact_summaries = tibble::tibble()))
  }

  keep_names <- taxa_names(ps)[keep]
  ps_filtered <- prune_taxa(keep_names, ps)
  taxa_keep_ids <- taxa_names(ps_filtered)
  tax_ph <- tax_tbl(ps_filtered)
  ...
  for (v in vars_to_analyze) {
    ...
    if (identical(network_mode, "compact")) {
      prep <- prepare_compact_sig_subset(sig_res_table, v, taxa_keep_ids, k_per_dir)
      sig_v <- prep$tbl
      if (nrow(prep$summary)) {
        desc_txt <- get_variable_description(v)
        gm <- ubiome_genus_mapping_complete %>%
          transmute(
            otu = .data$otu,
            map_genus = dplyr::coalesce(as.character(.data$parsed_genus), as.character(.data$Genus))
          )
        summ <- prep$summary %>%
          left_join(gm, by = "otu") %>%
          left_join(tax_ph %>% dplyr::select(OTU, Phylum), by = c("otu" = "OTU")) %>%
          mutate(
            dataset = ds_name,
            transformation = trans,
            host_variable = v,
            Variable.Description = desc_txt,
            raw_genus_label = dplyr::coalesce(as.character(.data$map_genus), as.character(.data$otu)),
            annotation_label_clean = purrr::map_chr(.data$raw_genus_label, function(rg) {
              rg <- as.character(rg)
              if (suppress_compact_annotation(rg)) {
                return(NA_character_)
              }
          ...
          dplyr::select(
            "dataset", "transformation", "host_variable", "Variable.Description",
            "otu", "estimate", "direction", "rank_within_direction",
            "Phylum", "raw_genus_label", "annotation_label_clean"
          )
        compact_parts[[v]] <- summ
      }
    }
  ...
  list(
    plot_list = plot_list,
    compact_summaries = dplyr::bind_rows(compact_parts)
  )
}
...
network_modes <- c("comprehensive", "compact")
grand_totals <- stats::setNames(rep(0L, length(network_modes)), network_modes)
all_compact_chunks <- list()

for (mode in network_modes) {
  for (tr in names(phylo_sets)) {
    ps <- phylo_sets[[tr]]
    tr_dir <- file.path(output_path, "network_plots_final", mode, tr)
    ...
    for (ds in names(datasets_corrected)) {
      if (ds %in% names(sig_results) && tr %in% names(sig_results[[ds]])) {
        ...
        plot_list <- res$plot_list
        dataset_plot_lists[[ds]] <- plot_list

        if (identical(mode, "compact") && nrow(res$compact_summaries)) {
          compact_for_tr[[ds]] <- res$compact_summaries
        }

        if (length(plot_list) > 0) {
          cat("  Saving", length(plot_list), "plots for", ds, "(", mode, ")\n")
          ...
          for (var_name in names(plot_list)) {
            ...
            grDevices::pdf(filename, 2.5, 2.5, family = pdf_font_family)
            print(plot_list[[var_name]]$plot)
            grDevices::dev.off()
          }
        } else {
          cat("  No plots created for", ds, "(", tr, ",", mode, ")\n")
        }

        grand_totals[[mode]] <- grand_totals[[mode]] + length(plot_list)
      }
    }

    if (identical(mode, "compact") && length(compact_for_tr)) {
      all_compact_chunks[[paste(mode, tr, sep = "_")]] <- dplyr::bind_rows(compact_for_tr)
    }
    ...
    if (length(dataset_order) > 0) {
      booklet_file <- file.path(tr_dir, paste0("ALL_", mode, "_", tr, "_network_booklet.pdf"))
      cat("Creating booklet:", booklet_file, "\n")

      max_grid_side <- max(1L, sapply(dataset_order, function(ds) {
      ...
      grDevices::pdf(booklet_file, width = max_grid_side * 2.5, height = max_grid_side * 2.5, family = pdf_font_family)
      blank_cell <- ggplot2::ggplot() + ggplot2::theme_void()

      for (ds in dataset_order) {
        plot_list_ds <- dataset_plot_lists[[ds]]
        grid_side <- if (length(plot_list_ds)) ceiling(sqrt(length(plot_list_ds))) else 1L
          ...
          grobs <- list(
            ggplot2::ggplot() +
              ...
              ggplot2::annotate(
                ...
                family = pdf_font_family, size = 5.5 / ggplot2::.pt
              )
          )
        }
        ...
        grid::grid.newpage()
        grid::grid.text(
          paste0(ds, " networks (", mode, ")"),
          y = grid::unit(0.98, "npc"),
          gp = grid::gpar(fontfamily = pdf_font_family, fontsize = 5.5, fontface = "bold")
        )

        content_vp <- grid::viewport(
          width = grid_side / max_grid_side,
...
compact_all <- dplyr::bind_rows(all_compact_chunks)
if (nrow(compact_all)) {
  out_csv <- file.path(tables_dir, "compact_top_taxa_all.csv")
  utils::write.csv(compact_all, out_csv, row.names = FALSE)
  message("Wrote compact taxon summary: ", out_csv)
}
...
message("Total comprehensive plots: ", grand_totals[["comprehensive"]])
message("Total compact plots: ", grand_totals[["compact"]])
