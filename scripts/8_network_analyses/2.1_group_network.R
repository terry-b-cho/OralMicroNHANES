...
# =============================================================================
# Variant of 2_network.R that groups multiple host variables into a single
# network panel (e.g. Respiratory_diseases = ASTHMA + EMPHYSEMA + BRONCHITIS).
# Extend the `group_specs` list at the top of the script to add more groups.
#
# All outputs are written under:
#   results/analyses_results/8_network_analyses_out/
#
# Environment: R >= 4.5 with phyloseq, igraph, ggraph, ggplot2, ggrepel,
# dplyr, tidyr, stringr, purrr, grid, gridExtra, extrafont, scales, rlang.
...
if (!dir.exists(data_path)) {
  stop("Intermediate data not found. Run 1_load_all_categories.R first.")
}
...
build_group_network <- function(group_name, spec, transformation) {
  ...
  adjusted_plot <- result$plot +
    labs(
      title = sprintf("%s — %s (%s)", group_label, dataset, transformation),
...
group_dir_base <- file.path(output_path, "group_networks")
...
for (group_name in names(group_specs)) {
  ...
  for (trans in transformations) {
    message("Processing group: ", group_name, " (", trans, ")")
    result <- build_group_network(group_name, spec, trans)
    if (is.null(result)) {
      next
    }
    ...
    pdf(filename, width = 2.5, height = 2.5, family = "Helvetica")
    print(result$plot)
    dev.off()
