...
## -----------------------------------------------------------------------------
## **REVIEWER_EDIT:** Consumes intermediates from
## `1_otu_host_and_indicies_intermediate_processing_REVIEWER_EDIT.R` only
## (under `4_otu_host_plot_out_REVIEWER_EDIT/intermediate/`). Writes all PDFs
## to `4_otu_host_plot_out_REVIEWER_EDIT/figures_out/{compact,comprehensive}/...`.
## Never write to `4_otu_host_plot_out/`. Do not edit the frozen
## `2_otu_host_n_indicies_plotting_n_saving.R` for reviewer work.
## -----------------------------------------------------------------------------
...
message("### Terry Plot Standalone Plotting (REVIEWER_EDIT) ###")
...
## Reviewer compact layer: top 5 positive + top 5 negative by |estimate| (same filtered results as full plot)
COMPACT_MAX_POS <- 5L
COMPACT_MAX_NEG <- 5L
...
## Axis/strip/base theme pt — match frozen Stage 2 script for compact ∩ comprehensive PDF overlays
AXIS_TEXT_PT <- 6L

## Styling knobs (REVIEWER_EDIT)
## - jitter is vertical only (width=0) so effect sizes are unchanged
## - compact jitter +10% vs prior 0.18 (reviewer pass)
...
## Match frozen script significance dot scale (was 0.80 for reviewer pass; restore 1 for overlay parity)
POINT_SIZE_SCALE <- 1
...
create_annotations <- function(df_plot, plot_type,
  ...
  if (plot_type == "otu") {
    ...
    # Reviewer request: compact plots should NOT annotate NA or Incertae_Sedis-like labels.
