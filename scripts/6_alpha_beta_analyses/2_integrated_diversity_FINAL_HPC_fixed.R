...
# Environment: R >= 4.5 with ggplot2, dplyr, tidyr, ape, egg, cowplot,
# data.table, ggdist, ggsignif (plus packages required by script 1, which is
# sourced internally). Conda spec: envs/nhanes-analysis_for_reviewers.yml.
...
NO_TICK_AND_BRACKET <- FALSE  # Set to TRUE to remove x-tick labels and brackets, FALSE for normal plots
NO_TICK_AND_BRACKET_ASTERISK <- FALSE  # Set to TRUE to show asterisks (*, **, ***) instead of exact P-values
HALF_PLOTS <- TRUE  # Set to TRUE for half violin plots
...
# Load data by sourcing the data loading script
cat("Loading data by sourcing data loading script...\n")
source("1_load_diversity_and_all_categories_HPC.R")

# Extract alpha diversity data
alpha_with_categories <- alpha_with_all_categories

if (TEST_MODE) {
  alpha_with_categories <- alpha_with_categories[1:min(TEST_SAMPLES, nrow(alpha_with_categories)), ]
}

# Filter categorical variables: allow up to 20 levels to include Outwas_all (17 levels) and AgeGroup_5years (11 levels)
categorical_vars <- names(alpha_with_categories)[sapply(alpha_with_categories, function(x) {
  is.factor(x) && nlevels(x) >= 1 && nlevels(x) <= 20 && sum(!is.na(x)) >= 30
})]
...
cat("  [done] Loaded beta diversity matrices\n")
...
for (metric_name in names(beta_metrics)) {
  ...
  cat("  Joining PCoA data for", metric_name, "...\n")
  flush.console()
  # Create PCoA data frame with just SEQN and coordinates first (faster)
  pcoa_df <- data.frame(
    SEQN = as.character(rownames(pcoa_res$vectors)),
    PC1 = pcoa_res$vectors[, 1],
    PC2 = pcoa_res$vectors[, 2],
    stringsAsFactors = FALSE
  )
  
  # Join only SEQN column from alpha_with_categories (we'll join full data later when needed)
  alpha_seqn_only <- alpha_with_categories %>% 
    mutate(SEQN = as.character(SEQN)) %>%
    select(SEQN, all_of(categorical_vars))
  
  pcoa_df <- pcoa_df %>%
    left_join(alpha_seqn_only, by = "SEQN")
...
cat("  [done] PCoA complete\n")
flush.console()

# Calculate centroids (optimized with batch processing)
cat("Calculating centroids...\n")
flush.console()

# Pre-allocate list to collect results (more efficient than rbind in loop)
...
for (metric_name in names(beta_metrics)) {
  ...
  for (cat_var in categorical_vars) {
    ...
    cat("    Variable:", cat_var, "(", match(cat_var, categorical_vars), "/", length(categorical_vars), ")\n")
    flush.console()
    ...
    for (group_level in unique_groups) {
      ...
      # Add to results list (more efficient than rbind)
      if (length(centroid_distances) > 0) {
        centroid_data_list[[list_idx]] <- data.frame(
          Metric = metric_name,
          Variable = cat_var,
...
if (length(centroid_data_list) > 0) {
  centroid_data_all <- do.call(rbind, centroid_data_list)
} else {
  centroid_data_all <- data.frame()
}

cat("  [done] Done (", nrow(centroid_data_all), " centroid distances calculated)\n", sep = "")
flush.console()
...
cat("  [done]", nrow(permanova_results), "tests,", sum(permanova_results$P_value < 0.05), "significant\n")
...
for (cat_var in categorical_vars) {
  ...
  for (metric_col in c("Observed_OTUs", "Shannon_Diversity", "Inverse_Simpson")) {
    ...
    if (HALF_PLOTS) {
      p <- ggplot(plot_data_alpha, aes(x = !!sym(cat_var), y = !!sym(metric_col), fill = !!sym(cat_var))) +
        geom_point(
          size = 0.1,
          alpha = 0.035,  # 30% more transparent than 0.05 (0.05 * 0.7 = 0.035)
        ...
        theme(
          plot.title = element_text(size = 6, face = "bold", hjust = 0.5, family = "Helvetica"),
          plot.subtitle = element_text(size = 6, hjust = 0.5, color = "gray30", family = "Helvetica"),
          axis.text.x = if(NO_TICK_AND_BRACKET) element_blank() else element_text(angle = 45, hjust = 1, size = 6, family = "Helvetica"),
      ...
      tryCatch({
        ...
        if (length(sig_pairs) > 0) {
          ...
          for (idx in seq_along(sig_pairs)) {
            ...
            # Add bracket with rounded corners and caps
            p <- p + 
              annotate("segment", x = pair$i, xend = pair$j, y = y_pos, yend = y_pos, size = 0.3, lineend = "round", linejoin = "round") +
              annotate("segment", x = pair$i, xend = pair$i, y = y_pos, yend = y_pos - y_range*0.03, size = 0.3, lineend = "round", linejoin = "round") +
              annotate("segment", x = pair$j, xend = pair$j, y = y_pos, yend = y_pos - y_range*0.03, size = 0.3, lineend = "round", linejoin = "round") +
              annotate("text", x = (pair$i + pair$j)/2, y = y_pos + y_range*0.025, 
                      label = p_label, size = 1.5, fontface = "plain", family = "Helvetica")
          }
      ...
      tryCatch({
        ...
        for (i in 1:(length(group_levels)-1)) {
          for (j in (i+1):length(group_levels)) {
            ...
            # Store ALL pairwise results for CSV - both beta_pairwise_results and beta_centroid_pairwise_results
            ...
            beta_centroid_pairwise_results <- rbind(beta_centroid_pairwise_results, result_row)
  ...
  for (metric_name in names(beta_metrics)) {
    ...
    # KEY FIXES: 
    # 1. Remove explicit 'group' parameter - let ggplot2 group automatically via color/fill
    # 2. Use single stat_ellipse call like working code
    # 3. Let centroids inherit colors by removing inherit.aes = FALSE and color = "black"
    p_pcoa <- ggplot(pcoa_data, aes(x = PC1, y = PC2, color = !!sym(cat_var), fill = !!sym(cat_var))) +
      ...
      stat_ellipse(geom = "polygon", alpha = 0.05, linetype = "dashed", linewidth = 0.4, level = 0.95, type = "t", segments = 200) + # 95% confidence regions - single ellipse like working code
      geom_point(data = centroids, aes(x = PC1_center, y = PC2_center, color = !!sym(cat_var)),
                 size = 1, shape = 3, stroke = 0.5, show.legend = FALSE) +  # Plus sign centroid - inherits color automatically
  ...
  for (metric_name in names(beta_metrics)) {
    ...
    p_scree <- ggplot(scree_data, aes(x = Axis, y = Variance)) +
      ...
      theme(
        plot.title = element_text(size = 6, face = "bold", hjust = 0.5, family = "Helvetica"),
        axis.text.x = element_text(angle = 45, hjust = 1, size = 6, family = "Helvetica"),
        axis.text.y = element_text(size = 6, family = "Helvetica"),
  ...
  # Dynamic PDF width and height based on number of factor levels
  ...
  # Dynamic height: base 180mm, increase by 15mm per level above 5 to accommodate brackets and annotations
  base_height_mm <- 180
  if (n_levels > 5) {
    additional_height_mm <- (n_levels - 5) * 15
    total_height_mm <- base_height_mm + additional_height_mm
  } else {
    total_height_mm <- base_height_mm
  }
  
  # If NO_TICK_AND_BRACKET mode, reduce height by 10% (no brackets needed)
  if (NO_TICK_AND_BRACKET) {
    total_height_mm <- total_height_mm * 0.9
  }
  
  pdf_width <- total_width_mm / 25.4    # Convert mm to inches
  pdf_height <- total_height_mm / 25.4  # Convert mm to inches
  ...
  dimensions <- paste0(total_width_mm, "×", round(total_height_mm), "mm")
  cat("  [done] ", filename, " (5×3 layout, ", dimensions, ")\n", sep = "")
}
...
if (nrow(alpha_pairwise_results) > 0) {
  write.csv(alpha_pairwise_results, 
            ...
            row.names = FALSE)
  cat("  [done] Saved alpha_pairwise_stat_res.csv (", nrow(alpha_pairwise_results), " comparisons)\n", sep = "")
...
if (nrow(beta_pairwise_results) > 0) {
  write.csv(beta_pairwise_results, 
            ...
            row.names = FALSE)
  cat("  [done] Saved beta_pairwise_stat_res.csv (", nrow(beta_pairwise_results), " comparisons)\n", sep = "")
  cat("    Significant comparisons:", sum(beta_pairwise_results$Significant, na.rm = TRUE), "\n")
}

# Save beta centroid PCoA pairwise results
if (nrow(beta_centroid_pairwise_results) > 0) {
  write.csv(beta_centroid_pairwise_results, 
            file.path(output_path, "beta_centroid_PCoA_pairwise_stat_res.csv"), 
            row.names = FALSE)
  cat("  [done] Saved beta_centroid_PCoA_pairwise_stat_res.csv (", nrow(beta_centroid_pairwise_results), " comparisons)\n", sep = "")
