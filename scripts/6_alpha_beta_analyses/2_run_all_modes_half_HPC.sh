...
#
# Environment: Slurm/HPC with conda/miniforge3 module. Activates the project
# conda env at $PROJECT_ROOT/envs/.conda/envs/nhanes-analysis.
...
cd "$PROJECT_ROOT/scripts/6_alpha_beta_analyses"
...
run_mode() {
    ...
    if [ $? -eq 0 ]; then
        echo "[done] $mode_name completed successfully"
        
        # Count generated PDF files
        pdf_count=$(find "$PROJECT_ROOT/results/analyses_results/6_alpha_beta_analyses_out/integrated_diversity_plots/" -name "*.pdf" | wc -l)
        echo "Generated $pdf_count PDF files"
    else
        echo "[ERROR] $mode_name failed"
        exit 1
    fi
...
# Final summary
total_pdfs=$(find "$PROJECT_ROOT/results/analyses_results/6_alpha_beta_analyses_out/integrated_diversity_plots/" -name "*.pdf" | wc -l)
