#!/usr/bin/env Rscript
################################################################################
##  4. INTEGRATED ALPHA & BETA DIVERSITY ANALYSIS - FINAL VERSION             ##
##  - 4×3 grid: Alpha + Beta centroid + PCoA + Scree                          ##
##  - Safe Grafify colors, dark red median, grayscale scree                   ##
##  - ALL 33 categorical variables (when TEST_MODE = FALSE)                   ##
##                                                                             ##
##  USAGE:                                                                     ##
##  - TEST MODE (fast): TEST_MODE <- TRUE (uses 500 samples, ~5 min)          ##
##  - FULL MODE (complete): TEST_MODE <- FALSE (uses 9,662 samples, ~45 min)  ##
################################################################################
...
cat("=== INTEGRATED DIVERSITY ANALYSIS - FINAL ===\n\n")
...
# │  TEST_MODE <- FALSE: Uses ALL 9,662 samples, ~30-45 min runtime │
# └─────────────────────────────────────────────────────────────────┘
...
NO_TICK_AND_BRACKET_ASTERISK <- TRUE  # Set to TRUE to show asterisks (*, **, ***) instead of exact P-values
    ...
    for (group_level in unique(alpha_with_categories[[cat_var]])) {
      ...
      for (sample in group_samples) {
        centroid_data_all <- rbind(centroid_data_all, data.frame(
          ...
          Distance_to_Centroid = mean(group_dist[sample, group_samples[group_samples != sample]])
        ))
      }
    }
  }
...
for (metric_name in names(beta_metrics)) {
  ...
  # Get samples that are in both distance matrix and alpha data
  common_samples <- intersect(labels(dist_obj), alpha_with_categories$SEQN)
  
  if (length(common_samples) < 30) next
  ...
  for (cat_var in categorical_vars) {
    ...
    tryCatch({
      formula_obj <- as.formula(paste("dist_subset ~", cat_var))
      ...
      cat("  Error for", metric_name, "-", cat_var, ":", e$message, "\n")
    })
  }
}
...
# Get top variables - handle empty permanova case
# FULL MODE: Use ALL categorical variables (not just top 10)
top_vars <- categorical_vars
...
for (cat_var in top_vars) {
  ...
  for (metric_col in c("Observed_OTUs", "Shannon_Diversity", "Inverse_Simpson")) {
    ...
    p <- ggplot(plot_data_alpha, aes(x = !!sym(cat_var), y = !!sym(metric_col), fill = !!sym(cat_var))) +
      ...
      geom_boxplot(width = 0.2, alpha = 0.2, outlier.shape = NA,
                   color = "darkred", fatten = 2) +  # DARK RED boxplot on TOP layer
          ...
          for (idx in seq_along(sig_pairs)) {
            ...
            p <- p + 
              ...
              annotate("text", x = (pair$i + pair$j)/2, y = y_pos + y_range*0.025, 
                      label = p_label, size = 1.5, fontface = "bold")
          }
  ...
  # BETA PLOTS (bottom row) - MATCHING colors with alpha
  for (metric_name in names(beta_metrics)) {
    ...
    p <- ggplot(beta_plot_data, aes(x = Group, y = Distance_to_Centroid, fill = Group)) +
      ...
      geom_boxplot(width = 0.2, alpha = 0.2, outlier.shape = NA,
                   color = "darkred", fatten = 2) +  # DARK RED boxplot on TOP layer
      scale_fill_manual(values = plot_colors) +  # SAME colors as alpha plots
  ...
  # ROW 3: PCoA PLOTS (from 3_beta_diversity_all_categories.R)
  for (metric_name in names(beta_metrics)) {
  ...
  # Combine plots in 4×3 grid (EXPANDED from 2×3)
  ...
  # Save with exact dimensions: 80mm × 135mm (or 10% shorter if NO_TICK_AND_BRACKET)
  pdf_width <- 80 / 25.4    # Convert mm to inches: 3.15 inches (width stays same)
  pdf_height <- if(NO_TICK_AND_BRACKET) (135 * 0.9) / 25.4 else 135 / 25.4  # 10% shorter if no tick and bracket
