...
##  - Replicates the standalone Terry-plot heatmaps with data-driven slicing   ##
##  - Computes silhouette-selected k for rows and columns per dataset          ##
##  - Preserves all aesthetics/dimensions while prefixing outputs              ##
################################################################################
...
con <- dbConnect(
  RSQLite::SQLite(),
  "/Users/byeongyeoncho/main/github/nhanes_oral_mirco_cho/data/00_nhanes_omp_transformed_db/nhanes_oral_transformed_complete_processed.sqlite"
)
...
transformations <- c("clr")  # ACTIVE - only CLR
# transformations <- c("clr", "lognorm")  # COMMENTED OUT - CLR + lognorm
...
if (!file.exists(sigres_path)) {
  sigres_path <- file.path(
    base_path,
    "results/analyses_results/4_otu_host_plot_out/intermediate/significant_results.rds"
  )
}
...
if (file.exists(sigres_path)) {
  ...
  get_host_variable_column <- function(analysis) {
    if (analysis %in% c("demoWAS", "exWAS")) {
      "independent_var"
    } else if (analysis %in% c("oradWAS", "pheWAS", "outWAS", "zimWAS")) {
      "dependent_var"
    } else {
      "term"
    }
  ...
  warning("Significant results file not found at ", sigres_path,
...
load_correlation_matrix <- function(analysis_type, transformation) {
  ...
  if (dataset_name == "allWAS_clr_all_results") {
    possible_paths <- c(
      file.path(microbial_signature_input, "merged_clr_data", "correlation_matrix.rds"),
      file.path(results_path, "analyses_results", "4_association_phyloseq_analyses_out", "intermediate", "microbial_signature", "merged_clr_data", "correlation_matrix.rds"),
      file.path(results_path, "analyses_results", "4_association_phyloseq_analyses_out", "intermediate", "arcive", "New Folder With Items", "merged_clr_data", "correlation_matrix.rds")
    )
  } else {
    possible_paths <- c(
      file.path(microbial_signature_input, dataset_name, "correlation_matrix.rds"),
      ...
      file.path(results_path, "analyses_results", "4_association_phyloseq_analyses_out", "intermediate", "microbial_signature", dataset_name, "correlation_matrix.rds"),
  ...
  for (transformation in transformations) {
    ...
    if (!is.null(res$cor_result)) {
      ...
      dataset_summary <- rbind(dataset_summary, data.frame(
        ...
        Analysis_Type = analysis_type,
        Transformation = transformation,
        N_Host_Variables = res$cor_result$n_variables,
        N_Correlations = res$cor_result$n_valid_correlations,
        N_Significant = res$cor_result$n_significant_correlations,
        Percent_Significant = round(res$cor_result$n_significant_correlations / res$cor_result$n_valid_correlations * 100, 1),
      ...
      Retained_Variables = ifelse(is.null(res$split_info$retained_variables), NA_integer_, res$split_info$retained_variables),
      FDR_Threshold = fdr_threshold,
      OTU_Threshold = otu_threshold,
      Prevalence_Threshold = prevalence_threshold,
        stringsAsFactors = FALSE
