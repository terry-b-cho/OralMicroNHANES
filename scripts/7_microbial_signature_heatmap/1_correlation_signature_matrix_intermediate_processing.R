...
## -----------------------------------------------------------------------------
## Builds per-analysis signature matrices, host-host weighted correlation
## matrices, merged (allWAS) signature/correlation outputs, and the
## heatmap_significant_results bundle consumed by the plotting scripts.
##
## All artefacts are written under
##   results/analyses_results/7_microbial_signature_heatmap_out/intermediate/
##
## Environment: R >= 4.5 with data.table, dplyr, tidyr, tibble, purrr,
## stringr, DBI, RSQLite. Conda spec: envs/nhanes-analysis_for_reviewers.yml.
## -----------------------------------------------------------------------------
...
create_merged_dataset <- function(signature_matrices,
  ...
  unwanted_sources <- c("demoWAS", "Unknown")
