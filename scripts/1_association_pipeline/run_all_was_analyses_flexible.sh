...
get_missing_dependent_variables() {
    ...
    echo "   Extracting expected dependent variables..." >&2
    ...
    echo "   File count: $total_existing/$total_expected exist" >&2
...
submit_analysis_type() {
    ...
    local missing_vars
    missing_vars=$(get_missing_dependent_variables "$analysis_type" "$normalization" "$test_mode")
    local exit_code=$?
...
if [[ ! -f "$ANALYSIS_R_SCRIPT" ]]; then
    echo "ERROR: Analysis R script not found: $ANALYSIS_R_SCRIPT"
    exit 1
fi
