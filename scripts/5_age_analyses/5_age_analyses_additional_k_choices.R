#!/usr/bin/env Rscript
# =============================================================================
# 5_age_analyses_additional_k_choices.R
#
# Additional age-stratified analyses, with extra per-k parallel-coordinate
# and PCA + t-SNE plots for k in {3, 7, 9, 12}:
...
#   2. Per-k parallel-coordinate (5.3) and combined PCA + t-SNE (5.4)
#      visualizations at k = 3, 7, 9, 12. The k = 9 outputs are produced by
#      the same code path / seeds / save_pdf_figure() parameters as
#      5_age_analyses_full.R so they reproduce that script's
#      5.3_parallel_coord_plot_faceted.pdf and 5.4_combined_pca_tsne.pdf.
#      The k = 3, 7, 12 outputs use a distinct colorblind-friendly cluster
#      palette to differentiate them from the k = 9 baseline.
#   3. GOLD-trait ecology summaries by genus cluster (trait distributions,
#      ternary cluster-mean summaries, connected ternary paths).
#   4. Nativity composition by age group (US Born / non-US Born proportions
#      across age bins, with chi-square / Fisher tests).
#
# Outputs are written under:
#   <PROJECT_ROOT>/results/analyses_results/5_age_analyses_out_additional_k_choices/
#     k_optimization/
...
#     5.3_parallel_coord_plot_faceted.pdf            (k = 9, baseline palette)
#     5.4_combined_pca_tsne.pdf                       (k = 9, baseline palette)
#     5.3_parallel_coord_plot_faceted_k{3,7,12}.pdf   (alt colorblind palette)
#     5.4_combined_pca_tsne_k{3,7,12}.pdf             (alt colorblind palette)
#     ecology/
#     nativity/
#
# Environment: R >= 4.5 with phyloseq, dplyr, tidyr, cluster, ggplot2, egg,
# patchwork, scales, grid, gridExtra, data.table, stringr, GGally, cowplot.
# (Rtsne is intentionally not used; see the Section 5b header for context.)
# Conda spec: envs/nhanes-analysis_for_reviewers.yml.
#
# Run from anywhere:
#   Rscript scripts/5_age_analyses/5_age_analyses_additional_k_choices.R
# =============================================================================
...
viz_out_root <- file.path(base_path,
                          ...
                          "5_age_analyses_out_additional_k_choices")
viz_out_path <- file.path(viz_out_root,
                          "k_optimization")

stopifnot(
  grepl("5_age_analyses_out_additional_k_choices", viz_out_path, fixed = TRUE),
...
cat("=== 5_age_analyses_additional_k_choices.R ===\n")
cat("base_path:    ", base_path,    "\n")
cat("viz_out_root: ", viz_out_root, "\n")
...
zero_norm <- nrm(0,            Wpp_min, Wpp_max)
...
# =============================================================================
# Section 5b - Per-k parallel-coordinate (5.3) and PCA + t-SNE (5.4) plots
#
# Renders 5.3 + 5.4 figures for k in {3, 7, 9, 12} off the same hc from
# Section 1. k = 9 reproduces 5_age_analyses_full.R's 5.3 + 5.4 byte-for-byte
# (same code path / seeds / save_pdf_figure params / Tol-light a..i palette).
# k = 3, 7, 12 share the same dendrogram cut and use a max-contrast 12-color
# cluster palette (Kelly-derived), with legends split into a companion
# *_legend.pdf and the main canvas shrunk to 65x72 mm so subpanels render
# square.
# =============================================================================
cat("\n=== SECTION 5b: per-k parallel-coord + PCA/tSNE plots ===\n")
...
# ---- Historical tSNE coordinate fixture ----
# Rtsne output drifts across BLAS / package builds, so we load the
# (genus, tsne_dim1, tsne_dim2) coords extracted once from
# 5_age_analyses_out/5.4_combined_pca_tsne.pdf rather than re-running Rtsne.
# This gives byte-identity at k = 9 and a shared tSNE pattern across k = 3, 7, 12.
# Regenerate via tools/extract_historical_tsne.py. Row order matches
# rownames(mean_abundance_scaled_full).
historical_tsne_csv <- file.path(
  base_path,
  "results/intermediate/historical_tsne_coords_k9.csv"
)
if (!file.exists(historical_tsne_csv)) {
  stop("Missing historical tSNE fixture: ", historical_tsne_csv,
       "\nRegenerate via tools/extract_historical_tsne.py.")
}
historical_tsne_df <- read.csv(historical_tsne_csv, stringsAsFactors = FALSE)
stopifnot(
  identical(historical_tsne_df$genus, rownames(mean_abundance_scaled_full)),
  all(c("genus", "tsne_dim1", "tsne_dim2") %in% colnames(historical_tsne_df))
)
cat(sprintf("[*] Loaded historical tSNE coords: %d rows from %s\n",
            nrow(historical_tsne_df), historical_tsne_csv))

phylum_colors_full <- c(
  "Firmicutes"             = "#F38400",
...
# Cluster palette for k = 3, 7, 12 - Kenneth Kelly's "maximum contrast"
# colors, ordered to maximize pairwise distinctness at low K.
cluster_palette_alt_12 <- c(
  "#FFB300",  # vivid yellow
...
tax_table_df_for_plots <- as.data.frame(tax_table(ubiome_filtered))

# 5 pt Helvetica RGB pdf saver matching 5_age_analyses_full.R's save_pdf_figure().
save_pdf_figure_into <- function(file_path, width_mm = 180, height_mm = 170) {
  pdf(
    file = file_path,
    width = width_mm / 25.4,
...
# K = 9 maps to the canonical a..i Tol-light palette; other K use numeric
# labels "1".."K" against cluster_palette_alt_12.
build_clusters_for_k <- function(K) {
  clusters_numeric <- cutree(hc, k = K)
  if (identical(as.integer(K), 9L)) {
    cl_factor <- factor(cluster_mapping_main[as.character(clusters_numeric)],
                        levels = cluster_letters_a_to_i)
    cl_colors <- cluster_colors_a_to_i
  } else {
    cl_factor <- factor(as.character(clusters_numeric),
                        levels = as.character(seq_len(K)))
    cl_colors <- setNames(cluster_palette_alt_12[seq_len(K)],
                          as.character(seq_len(K)))
  }
  list(clusters = cl_factor, colors = cl_colors)
}

# Plot code / seeds / save_pdf_figure params mirror 5_age_analyses_full.R.
# out_5_3_path NULL: skip the 5.3 parallel-coord plot (use when re-rendering
# 5.4 for the same K under a different legend split). out_5_4_legend_path
# NULL: legends embedded in each subpanel; non-NULL: legends stripped from
# the panel grid and written to that path as a separate legend-only PDF.
render_5_3_and_5_4_for_k <- function(K,
                                     out_5_3_path,
                                     out_5_4_path,
                                     out_5_4_legend_path = NULL) {
  obj <- build_clusters_for_k(K)
  clusters_factor      <- obj$clusters
  cluster_colors_local <- obj$colors

  mean_abundance_df <- as.data.frame(mean_abundance_scaled_full)
  mean_abundance_df$genus   <- rownames(mean_abundance_scaled_full)
  mean_abundance_df$cluster <- clusters_factor
  mean_abundance_df$phylum  <- tax_table_df_for_plots[mean_abundance_df$genus,
                                                     "Phylum"]

  if (!is.null(out_5_3_path)) {
    p_parallel <- GGally::ggparcoord(
      data         = mean_abundance_df,
      columns      = 1:(ncol(mean_abundance_df) - 3),
      groupColumn  = "phylum",
      scale        = "globalminmax",
      ...
      scale_color_manual(values = phylum_colors_full) +
      egg::theme_article() +
      theme(
      ...
      labs(x = "Age Group", y = "Standardized Relative Abundance",
           color = "Phylum")

    save_pdf_figure_into(out_5_3_path, width_mm = 100, height_mm = 90)
    print(p_parallel)
    dev.off()
  ...
  set.seed(42)
  pca_result <- prcomp(mean_abundance_scaled_full, scale. = FALSE)
  pca_df <- as.data.frame(pca_result$x[, 1:2])
  pca_df$genus   <- rownames(mean_abundance_scaled_full)
  pca_df$cluster <- clusters_factor
  ...
  p_pca_cluster <- ggplot(pca_df, aes(x = PC1, y = PC2, color = cluster)) +
    ...
    scale_color_manual(values = cluster_colors_local) +
    egg::theme_article() +
    labs(title = "PCA of Genera by Cluster Assignment",
         x = pc1_label, y = pc2_label, color = "Cluster") +
    theme(
      plot.title   = element_text(size = 6, face = "bold", hjust = 0.5),
      axis.title   = element_text(size = 6),
  ...
  p_pca_phylum <- ggplot(pca_df, aes(x = PC1, y = PC2, color = phylum)) +
    ...
    scale_color_manual(values = phylum_colors_full) +
    egg::theme_article() +
    labs(title = "PCA of Genera by Phylum",
         x = pc1_label, y = pc2_label, color = "Phylum") +
    theme(
      plot.title   = element_text(size = 6, face = "bold", hjust = 0.5),
      axis.title   = element_text(size = 6),
      axis.text    = element_text(size = 6),
  ...
  tsne_df <- data.frame(
    Dim1    = historical_tsne_df$tsne_dim1,
    Dim2    = historical_tsne_df$tsne_dim2,
    genus   = historical_tsne_df$genus,
    stringsAsFactors = FALSE
  )
  tsne_df$cluster <- clusters_factor
  ...
  p_tsne_cluster <- ggplot(tsne_df, aes(x = Dim1, y = Dim2, color = cluster)) +
    ...
    scale_color_manual(values = cluster_colors_local) +
    egg::theme_article() +
    labs(title = "t-SNE of Genera by Cluster Assignment",
  ...
  p_tsne_phylum <- ggplot(tsne_df, aes(x = Dim1, y = Dim2, color = phylum)) +
    ...
    scale_color_manual(values = phylum_colors_full) +
    egg::theme_article() +
    labs(title = "t-SNE of Genera by Phylum",
         x = "t-SNE Dimension 1", y = "t-SNE Dimension 2",
         color = "Phylum") +
    theme(
      plot.title   = element_text(size = 6, face = "bold", hjust = 0.5),
  ...
  if (is.null(out_5_4_legend_path)) {
    ...
    combined_plot <- cowplot::plot_grid(
      p_pca_cluster  + theme(legend.position = "none"),
      p_pca_phylum   + theme(legend.position = "none"),
      p_tsne_cluster + theme(legend.position = "none"),
      p_tsne_phylum  + theme(legend.position = "none"),
      labels = c("A", "B", "C", "D"),
      ncol = 2, label_size = 14
    )

    # Cluster legend wraps to 2 rows once K > 6 to avoid horizontal overflow.
    cluster_nrow <- if (K <= 6L) 1L else 2L
    leg_cluster <- cowplot::get_legend(
      p_pca_cluster +
        theme(legend.position = "bottom",
              legend.title = element_text(size = 6, face = "bold"),
              legend.text  = element_text(size = 6),
              legend.margin = margin(t = 0, b = 0, l = 4, r = 4)) +
        guides(color = guide_legend(title = "Cluster", nrow = cluster_nrow,
                                    override.aes = list(size = 2)))
    )
    leg_phylum <- cowplot::get_legend(
      p_pca_phylum +
        theme(legend.position = "bottom",
              legend.title = element_text(size = 6, face = "bold"),
              legend.text  = element_text(size = 6),
              legend.margin = margin(t = 0, b = 0, l = 4, r = 4)) +
        guides(color = guide_legend(title = "Phylum", nrow = 2,
                                    override.aes = list(size = 2)))
    )
    legend_only <- cowplot::plot_grid(
      leg_cluster, leg_phylum,
      ncol = 1, rel_heights = c(cluster_nrow, 2)
    )

    leg_height_mm <- if (cluster_nrow == 1L) 35 else 45
    save_pdf_figure_into(out_5_4_legend_path,
                         width_mm = 170, height_mm = leg_height_mm)
    print(legend_only)
    dev.off()
    cat(sprintf("[saved] %s (k = %d, legend only)\n",
                out_5_4_legend_path, K))
  }

  # 100 mm at k = 9 keeps the historical layout for byte-identity; 72 mm
  # at k = 3 / 7 / 12 makes the legend-stripped subpanels square.
  main_height_mm <- if (is.null(out_5_4_legend_path)) 100 else 72
  save_pdf_figure_into(out_5_4_path, width_mm = 65, height_mm = main_height_mm)
  print(combined_plot)
  dev.off()
  cat(sprintf("[saved] %s (k = %d)\n", out_5_4_path, K))
}

render_5_3_and_5_4_for_k(
  K = 9L,
  out_5_3_path = file.path(viz_out_root, "5.3_parallel_coord_plot_faceted.pdf"),
  out_5_4_path = file.path(viz_out_root, "5.4_combined_pca_tsne.pdf")
)

# k = 9 again, legend-split (square 65x72 mm panels + companion legend PDF),
# using the same Tol-light a..i cluster palette as the baseline.
render_5_3_and_5_4_for_k(
  K = 9L,
  out_5_3_path = NULL,
  out_5_4_path        = file.path(viz_out_root, "5.4_combined_pca_tsne_k9.pdf"),
  out_5_4_legend_path = file.path(viz_out_root, "5.4_combined_pca_tsne_k9_legend.pdf")
)

for (K in c(3L, 7L, 12L)) {
  render_5_3_and_5_4_for_k(
    K = K,
    out_5_3_path = file.path(viz_out_root,
                             sprintf("5.3_parallel_coord_plot_faceted_k%d.pdf", K)),
    out_5_4_path = file.path(viz_out_root,
                             sprintf("5.4_combined_pca_tsne_k%d.pdf", K)),
    out_5_4_legend_path = file.path(viz_out_root,
                             sprintf("5.4_combined_pca_tsne_k%d_legend.pdf", K))
  )
}

cat("=== SECTION 5b: per-k 5.3 and 5.4 plots complete ===\n")
...
viz_out_root <- file.path(base_path, "results/analyses_results/5_age_analyses_out_additional_k_choices")
